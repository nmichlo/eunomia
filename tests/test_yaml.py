...
def test_yaml_tags():
    ...
    assert yaml_load('f""').get_value({}, {}) == ''
    assert yaml_load('\'f""\'').get_value({}, {}) == ''
    assert yaml_load('f"{1}"').get_value({}, {}) == '1'
    assert yaml_load('f"{1}{[1,2]}"').get_value({}, {}) == '1[1, 2]'
    assert yaml_load('f"{1}{[1,2]*2}"').get_value({}, {}) == '1[1, 2, 1, 2]'
    assert yaml_load('f"| {\'=\'*5} |"').get_value({}, {}) == '| ===== |'
    assert yaml_load('f"| {1+2+0.56:1.1f} |"').get_value({}, {}) == '| 3.6 |'
    assert yaml_load('f"{round(0.5)}"').get_value({}, {}) == '0'
